...
export default function DashboardPage() {
  ...
  const fuse = useMemo(() => { if (!rawSubmissions.length) return null; return new Fuse(rawSubmissions, { keys: ['mainText', 'locationName', 'machineName', 'submitterName', 'machineSerialNumber'], includeScore: true, threshold: 0.4, minMatchCharLength: 2 }); }, [rawSubmissions]);
  const displaySubmissions = useMemo(() => { if (debouncedSearchQuery.trim() && fuse) { return rawSubmissions; } return rawSubmissions; }, [debouncedSearchQuery, rawSubmissions, fuse]);

  useEffect(() => { if (activeTab === null && isAdmin !== undefined && !debouncedSearchQuery) { setActiveTab(isAdmin ? 'needsReview' : 'serviceReports'); } }, [isAdmin, activeTab, debouncedSearchQuery]);

  useEffect(() => { if(!debouncedSearchQuery) { setComplaintStatusFilter('all'); setFeedbackStatusFilter('all'); } }, [activeTab, debouncedSearchQuery]);
  useEffect(() => { if (debouncedSearchQuery) { setActiveTab(null); } }, [debouncedSearchQuery]);

  const handleReviewClick = useCallback(() => { setSearchQuery(''); setActiveTab('needsReview'); }, []);
  const handleViewSubmission = useCallback((submission: EnrichedReport) => setSelectedSubmission(submission), []);
  const handleTabClick = (tabKey: TabType) => { setSearchQuery(''); setActiveTab(tabKey); }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <main className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
              {isAdmin ? (
                <div className="w-full flex flex-col sm:flex-row gap-2 items-center">
                  ...
                  {/* --- MODIFIED: Wrapped dropdown in a div with a fixed width for larger screens --- */}
                  <div className="w-full sm:w-56">
                    <CategoryDropdown options={categoryTabs} selected={activeTab} onSelect={handleTabClick} />
                  </div>
                  
                  {/* --- MODIFIED: Added max-width to the search field for larger screens --- */}
                  <div className="relative flex-grow w-full sm:max-w-md">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><Search className="h-5 w-5 text-gray-400" /></div>
                    <input type="text" placeholder="Search problems, clients, authors..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="block w-full pl-10 pr-10 py-2.5 border border-gray-300 rounded-lg bg-white placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                ...
                <div className="w-full flex flex-col md:flex-row gap-4 items-center">
                  ...
                  <div className="relative w-full md:max-w-xs ml-auto"><div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><Search className="h-5 w-5 text-gray-400" /></div><input type="text" placeholder="Search approved reports..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="block w-full pl-10 pr-10 py-2.5 border border-gray-300 rounded-lg bg-white placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />{searchQuery && (<div className="absolute inset-y-0 right-0 pr-3 flex items-center"><button onClick={() => setSearchQuery('')} className="text-gray-400 hover:text-gray-600"><X className="h-5 w-5" /></button></div>)}</div>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="p-4 sm:p-6">
            {submissionsData === undefined ? ( <TableSkeleton /> ) : (
              <DashboardForm 
