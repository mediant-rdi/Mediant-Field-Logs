...
export default function ServiceDelayForm() {
  ...
  const handleFinalSubmit = async () => {
    // UPDATED: This guard clause fixes the TypeScript error.
    // It proves to TypeScript that `formData.problemType` is not an empty string
    // before it's passed to the mutation, and adds a layer of robustness.
    if (formData.problemType === '') {
      alert("A problem type must be selected. Please go back and edit your report.");
      setIsReviewing(false); // Send user back to the form
      return;
    }
      ...
      {isReviewing ? (
        <div className="review-container">
            ...
            <div className="review-grid">
              ...
              <div className="review-item"><strong>Problem Type:</strong><p style={{textTransform: 'capitalize'}}>{formData.problemType.replace('-', ' ')}</p></div>
              
              {formData.problemType === 'service-delay' && (
                 <div className="review-item full-width">
                    <strong>Service Delay Details:</strong>
                    <ul>
                      {formData.backofficeAccess && <li>Delayed backoffice access</li>}
                      {formData.spareDelay && <li>Spare delay</li>}
                      {formData.delayedReporting && <li>Delayed reporting</li>}
                      {formData.communicationBarrier && <li>Communication barrier</li>}
                    </ul>
                 </div>
              )}
               {formData.problemType === 'other' && (
                 <div className="review-item full-width"><strong>Specific Problem:</strong><p>{formData.otherText}</p></div>
              )}
