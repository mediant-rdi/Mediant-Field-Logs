...
export default function Header({ onMenuClick, pageTitle }: HeaderProps) {
  ...
  // This query now runs for ALL authenticated users, including admins.
  const userNotifications = useQuery(api.notifications.getUnreadNotifications, isAuthenticated ? {} : "skip") as Notification[] ?? [];
  const userUnreadCount = userNotifications.length;
  
  // An admin's total notification count is the sum of admin tasks and personal tasks.
  const notificationCount = isAdmin ? adminPendingCount + userUnreadCount : userUnreadCount;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Mark as read only if there were personal notifications to read.
      if (wasOpen && userUnreadCount > 0) {
        markAsRead();
      }
    };
  }, [notificationsOpen, userUnreadCount, markAsRead]);
  ...
  const mediaQueries = `
    ...
    @media (max-width: 640px) { .header-container { padding: 12px 16px; } .header-left-side { gap: 8px; } .header-title { font-size: 18px; } .header-right-side { gap: 8px; } .notification-dropdown { position: fixed; top: 60px; left: 50%; transform: translateX(-50%); width: 90vw; max-width: 400px; right: auto; margin-top: 0; } .notification-item-text { white-space: normal; word-break: break-word; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; text-overflow: ellipsis; } }
  `;
                    ...
                    <>
                        ...
                        {isAdmin && adminPendingCount > 0 && (
                            <div style={{ padding: '12px 16px', borderBottom: '1px solid #f3f4f6' }}>
                                <p style={{ margin: 0, fontSize: '14px', color: '#111827' }}>You have {adminPendingCount} submission{adminPendingCount > 1 ? 's' : ''} pending approval.</p>
                                <Link href="/dashboard" onClick={() => setNotificationsOpen(false)} style={{ display: 'inline-block', marginTop: '8px', fontWeight: 500, color: '#2563eb', textDecoration: 'none' }}>Go to Dashboard</Link>
                            </div>
                        )}

                        {/* Personal notifications (for assignments, approvals, etc.) */}
